...
from src.o2_config import train_batch_size, MAX_NB_VARIABLES, batch_size, NB_CLASS, \
    model_folder, train_tmp, epochs, units, n_splits
...
def get_model():
    model = Sequential()
    model.add(LSTM(input_shape=(MAX_NB_VARIABLES, batch_size), units=units))
    model.add(Dense(NB_CLASS, activation='softmax'))
    model.summary()
    optimize = keras.optimizers.Adam(lr=0.02, beta_1=0.9, beta_2=0.999, decay=0.01)
...
def train_lstm():
    ...
    for train_index, test_index in skf_cv.split(X, y):
        ...
        model.fit(X_train, y_train, batch_size=30000, epochs=100, verbose=1, class_weight=weight_dict)
