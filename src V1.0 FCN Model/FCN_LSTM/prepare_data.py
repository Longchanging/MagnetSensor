...
def train_test_evalation_split(data, label):
    ...
    X_train, X_test, y_train, y_test = train_test_split(data, label, test_size=test_ratio, random_state=0, shuffle=False)
...
def fetch_batch_data(data, n_classes, batch_size):
    ...
    for i in range(n_classes):  # excluded label
        ...
        print('label shape: \t', label_class_i.shape, '\t data shape: \t', data_class_i.shape)
