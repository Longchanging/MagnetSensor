...
from src.src.config import model_folder, use_pca, saved_dimension_after_pca, \
...
def read_single_txt_file_new(single_file_name):
    ...
    '''   
    # print('in')
    ''' Should greatly change the resampling method'''
    read_array = np.loadtxt(single_file_name, skiprows=0) 
    r, c = read_array.shape
    read_array = read_array.reshape([r * c, 1])
