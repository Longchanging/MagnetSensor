...
'''
from config import train_folder, test_folder, predict_folder, \
...
def read_single_txt_file(single_file_name):
    ...
    #重新定义，10ms一个窗口，即原来10行一个窗口
    read_lines = int(len(file_list)/10)
    file_list = file_list[:read_lines*10].reshape([read_lines,10])
...
def preprocess_One_Array(array_, category, after_fft_data):
    ...
    while(i * overlap_window + window_length < rows):  # attention here
        ...
        # numerical feature
        numerical_feature_tmp = tmp_window #numericalFeather(tmp_window)
