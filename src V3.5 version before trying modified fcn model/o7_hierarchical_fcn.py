...
from src.o2_config import MAX_NB_VARIABLES, batch_size, NB_CLASS, \
    model_folder, train_tmp, n_splits, evaluation_ratio, whether_shuffle_train_and_test
...
def generate_model():
    ...
    ip1 = Input(shape=(MAX_NB_VARIABLES, batch_size))
    ip2 = Input(shape=(MAX_NB_VARIABLES, batch_size))
...
def train_fcn():
    ...
    for train_index, test_index in skf_cv.split(X, y):
        ...
        model.fit(x=[X_train, X_train], y=y_train, batch_size=train_batch_size, epochs=epochs, callbacks=callback_list,
                  class_weight=weight_dict, verbose=1, validation_data=([X_validate, X_validate], y_validate))
        
        predict_y = model.predict([X_test, X_test * 2]) 
    ...
    # ## 使用全部数据，使用保存的，模型进行实验
    predict_y_left = model.predict([X_test_left, X_test_left * 2])  # now do the final test
