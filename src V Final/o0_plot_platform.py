...
M, N = 10000, 800000  # 6300, 15000  # M定义数据从第几行开始读，N代表读多少行
ML = 10
sigma = 3
M = 4  # 绘制几张子图
linewidth = 2
...
def interact_With_User():
    ...
    train_keyword = {
        ...
        # 'platform':['mac__ms_word_1.txt', 'mac__ms_word_2.txt', 'mac__ms_word_3.txt', 'mac__iina_offline_video_2.txt'],
        'platform':['mac__netmusic_app_1.txt', 'mac__safari_surfing_1.txt', 'mac__safari_web_live_video_1.txt', 'shenzhou_game_myworld_1.txt'],
        # 'platform':['hp_powerpoint_2.txt', 'hp_edge_surfing_1.txt', 'mac__safari_surfing_1.txt', 'shenzhou_chrome_surfing_1.txt'],
        'platform':['mac__netmusic_app_1.txt']  # , 'hp_powerpoint_2.txt', 'hp_powerpoint_3.txt'],
...
def plot(file_, values, i):
    ...
    # ax.legend(loc='best')
    plt.savefig(image_folder + '%s%d.png' % (file_.split('.')[0], i))
    # plt.show()
...
def data_explore(file_all):
    ...
    # 重新制定归一化和反向测量
    a = np.max(np.max(file_all))
    b = np.min(np.min(file_all))
    c = maxMI
    d = minMI
    x = file_all
    # 反相
